...
# variables to update
query = '_sourcecategory=* | timeslice 1m | count by _timeslice'
lookback = 15 # minutes
...
def check(search_id):
    ...
    gathering = True
    df = pd.DataFrame(columns=['count', 'timeslice']) # need to update based on columns desired 
    while gathering:
            ...
            if query_type == 'agg':
                while offset < total_records:
                    ...
                    for r in results.json()['records']:
                        row = dict()
                        row['count'] = r['map']['_count']
                        row['timeslice'] = r['map']['_timeslice']
...
if __name__ == '__main__':
    ...
    search_id = start()
    data = check(search_id)  
    # basic stats for dataframe
